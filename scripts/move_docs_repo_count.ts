...
async function main() {
  ...
  const collectionFrom = 'AggregatedReposNext-count';
  const collectionTo = 'AggregatedRepos-count';

  const repoData = await findInMongoDb(
    connectionString,
    databaseName,
    collectionFrom,
    query,
  ...
  const uploadedResult = await uploadArrToMongoDb(
    connectionString,
    databaseName,
    collectionTo,
    repoData
