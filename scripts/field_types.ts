import {
  ...
  async function main() {
    ...
    const results = await client.db(databaseName).collection(collectionName).aggregate( 
        [ 
            { "$project": { _id:0, "fieldType": {  "$type": "$customDateUploaded"  } } },
            {"$group": { _id: {"fieldType": "$fieldType"},count: {$sum: 1}}}
        ]).toArray();
    client.close();
    return results;
  }
  main().then((results)=>console.log(results)).catch((err) => console.log(err));
